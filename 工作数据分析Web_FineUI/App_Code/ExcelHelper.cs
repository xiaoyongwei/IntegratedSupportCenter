   ...
   public  class ExcelHelper : IDisposable
    {
       ...
       // /// <param name="isSpace">是否允许存在空格</param>
        /// <returns>返回的DataTable</returns>
        public DataTable ExcelToDataTableDaoru( bool isFirstRowColumn)
        {
            ISheet sheet = null;
            DataTable data = new DataTable();
            int startRow = 0;
            ...
            try
            {
                ...
                if (sheet != null)
                {
                    IRow firstRow = sheet.GetRow(2);//从第3行开始,第3行是标题,第4行开始是数据
                    int cellCount = firstRow.LastCellNum; //一行最后一个cell的编号 即总的列数

                    //数据至少需要4行,即sheet.LastRowNum必须大于等于3
                    ...
                    for (int i = startRow; i <= rowCount; ++i)
                    {
                        ...
                        for (int j = row.FirstCellNum; j < cellCount; ++j)
                        {
                            ...
                            if (icell != null) //同理，没有数据的单元格都默认是null
                            {
                                    dataRow[j] =  icell.ToString().Replace(" ", "");
                            }
                        }
                        data.Rows.Add(dataRow);
                    }
