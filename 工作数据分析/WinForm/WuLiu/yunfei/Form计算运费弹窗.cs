        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            if (baodi > 0 && diquYunfei > 0 && IDList.Count > 0)
            {
                ...
                double sumAreas = Convert.ToDouble(OracleHelper.ExecuteScalar(
                    ...
                    "and i.orgcde = t.orgcde and i.PONO = t.pono))FROM FERP.DLV_FARE t WHERE id IN(" + idIn + ")"));
                double zongYunfei = 0;
                if (sumAreas >= baodi)
                {
                ...
                string SQL = string.Format("UPDATE FERP.DLV_FARE t SET ACCAMT =round( (select nvl(sum(i.ratios * i.acreage * i.ACCNUMR),0) from v_bcdx_ct i " +
