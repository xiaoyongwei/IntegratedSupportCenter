...
namespace 综合保障中心.其它
{
    public partial class FormYijie_自动获取 : Form
    {
        ...
        {
            if (!webBrowser.Url.OriginalString.Contains("ej-sh.net:9191/dlvFare.shtml?method:form="))
            {
        ...
        private void Get销售退货_明细()
        {
            ...
            if (hec.Count > 2)
            {
                ...
                for (int trI = 2; trI < hec.Count; trI++)
                {
                    ...
                    //退货单号
                    listObj.Add(webBrowser.Document.GetElementById("edat.pono").GetAttribute("value"));
                    //客户信息
                    listObj.Add(webBrowser.Document.GetElementById("clntnme").GetAttribute("value"));
                    //操作日期
                    listObj.Add(webBrowser.Document.GetElementById("edat.ptdate").GetAttribute("value"));
                    dt.Rows.Add(listObj.ToArray());
                }
                ...
                //开始添加到sql中
                List<string> sqlList = new List<string>();
                StringBuilder sb_Insert = new StringBuilder("replace  INTO `slbz`.`销售退货_明细`(");
                foreach (DataColumn dc in dt.Columns)//添加列
                {
                    sb_Insert.AppendFormat("`{0}`,", dc.ColumnName);
                }
            ...
            if (hec.Count > 2)
            {
                ...
                for (int trI = 2; trI < hec.Count; trI++)
                {
                    ...
                    foreach (HtmlElement item in hec[trI].GetElementsByTagName("td"))
                    {
                        ...
                        && string.IsNullOrWhiteSpace(item.OuterText)
                            && Regex.IsMatch(item.InnerHtml, "value=\\d+"))
                        {
                            listObj.Add(Regex.Match(Regex.Match(item.InnerHtml, "value=\\d+").Value, "\\d+"));
                        }
                        else
                        {
                            listObj.Add(item.OuterText);
                ...
                //开始添加到sql中
                List<string> sqlList = new List<string>();
                StringBuilder sb_Insert = new StringBuilder("replace  INTO `slbz`.`销售退货_概况`(");
                foreach (DataColumn dc in dt.Columns)//添加列
                {
                    sb_Insert.AppendFormat("`{0}`,", dc.ColumnName);
                }
        ...
        private void Get计划物流综合查询()
        {
            ...
            if (hec.Count > 2)
            {
                ...
                //开始添加到sql中
                List<string> sqlList = new List<string>();
                StringBuilder sb_Insert = new StringBuilder("replace  INTO `slbz`.`计划物流综合查询`(");
                foreach (DataColumn dc in dt.Columns)//添加列
                {
                    sb_Insert.AppendFormat("`{0}`,", dc.ColumnName);
                }
        ...
        private void GetKucunMingxi()
        {
            ...
            if (hec.Count > 2)
            {
                ...
                {
                    AddtbShow("获取[库存明细]成功! " + dic.Count);
                }
            }
            wAfter = WebAfter.下个明细;
        ...
        private void GetSonghuoMingxi()
        {
            ...
            if (hec.Count > 2)
            {
                ...
                {
                    AddtbShow("获取[送货记录]成功! " + dic.Count);
                }
            }
