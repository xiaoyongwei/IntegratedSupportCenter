...
namespace HandeJobManager.DAL
{
    ...
    public static  class SQLiteDbHelper
    {
        ...
        /// 连接字符串
        /// </summary>
        public readonly static string ConnectionString = @"Data Source="+Application.StartupPath+"\\Data\\外购对账.db;Version=3;";
        ...
        /// <param customerName="commandType">执行类型</param>
        /// <returns>SqlDataReader对象</returns>
        public static SQLiteDataReader ExecuteReader(string commandText, CommandType commandType)
        {
            SQLiteDataReader reader = null;
            if (ConnectionString == null || ConnectionString.Length == 0)
                throw new ArgumentNullException("ConnectionString");
        ...
        #region static  ExecuteDataTable
        ...
        public static DataTable SelectPaging(string tableName, string strColumns, string strWhere, string strOrder, int pageSize, int currentIndex)
        {
        ...
        {
            int num = 0;
            while (!ExecuteSqlTran_Ex(new List<string>(new string[] { SQLString })))
            {
                if (num++ < 12)
                {
                    //"失败,300毫秒后重试!"
