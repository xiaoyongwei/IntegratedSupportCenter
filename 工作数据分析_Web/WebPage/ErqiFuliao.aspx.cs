...
public partial class WebPage_ErqiFuliao : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        string sqlstr="SELECT *	FROM `slbz`.`金蝶_生产领料` where 日期 between "
                +Calendar_S.SelectedDate.ToString("yyyy-MM-dd")+" and "
                +Calendar_E.SelectedDate.ToString("yyyy-MM-dd");
        ...
        + "FROM `slbz`.`金蝶_生产领料`where 日期 between '" + Calendar_S.SelectedDate.ToString("yyyy-MM-dd")
        + "' and '" + Calendar_E.SelectedDate.ToString("yyyy-MM-dd") + "')aa";
